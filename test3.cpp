...
typedef struct tblock{      //传送节点坐标
    int row,count;
}tblock;
typedef struct tsend{       //主进程发送结构体
    tblock now;             //开始坐标
    int value,weight;       //当前的物品属性
    long long last_date[package_size + 1];      //上一层的数据
}tsend;
typedef struct tresv{       //主进程接收结构体
    tblock now;             //开始坐标
    long long AU[block];    //处理完成块
}tresv;
typedef struct nod{         //正在处理的块节点记录
    tblock value;
    nod* next;
}nod;
typedef struct status{      //当前状态结构体
    tblock next_livelast;
    nod *root;
    nod *last_nod;
}status;
typedef struct content{     //相关数据的结构体
    int value[item_N],weight[item_N];
    long long dp[3][package_size+1];
}content;
MPI_Datatype create_trans(){//创建传输类型
    MPI_Aint base_addr;
    MPI_Aint disp[2] = {0};
    int blocklen[2] = {1, 1};
...
MPI_Datatype create_tsend(){//创建传输类型
    MPI_Aint base_addr;
    MPI_Aint disp[4] = {0};
...
MPI_Datatype create_tresv(){//创建传输类型
    MPI_Aint base_addr;
    MPI_Aint disp[2] = {0,0};
    int blocklen[2] = {1,package_size + 1};
...
bool get_next(tsend &resv,tresv &send){//从进程获取任务
    MPI_Send(&send,1,tresv_type,0,0,MPI_COMM_WORLD);    //将结果发送给主进程
    MPI_Status status;
    MPI_Recv(&resv,1,tsend_type,0,0,MPI_COMM_WORLD,&status);
    return resv.now.row < item_N;
}
void step(tsend &resv,tresv &send){//从进程处理任务
    // cout<<myid<<": my pa is "<<state->dp[0][1]<<endl;
    int value = resv.value;
    int weight = resv.weight;
    send.now.row=resv.now.row;
    send.now.count=resv.now.count;
    //     cout<<"__________"<<send.now.row<<","<<send.now.count<<endl;
    //     for(long long i:resv.last_date)cout<<i<<" ";cout<<endl;
    // }    
    // cout<<endl<<send.now.row<<","<<send.now.count<<"_______________"<<weight<<","<<value<<endl;
    ...
    // cout<<endl;
    // for(long long i:send.AU)cout<<i<<" ";cout<<endl<<endl;
}
void delete_node(status &Status,tblock &t){//主进程删除已经完成的任务
    nod *now = Status.root;
...
void add_node(status &Status,tblock &t){//主进程创建任务
    if(Status.root == NULL){    //如果不存在未完成的任务，要创建新任务
...
void merge_ans(content *cont,tresv &Bresv){ //将结果同步到内存中
    if(Bresv.now.row!=-1&&Bresv.now.count!=-1){
        memcpy(&(cont->dp[(Bresv.now.row)%2][Bresv.now.count + 1]),&(Bresv.AU),sizeof(long long)*block);
    }
    // cout<<"____________"<<Bresv.now.row<<","<<Bresv.now.count<<"_______"<<endl;
    // for(long long i:Bresv.AU)cout<<i<<" ";cout<<endl;
    // cout<<"____________"<<Bresv.now.row<<","<<Bresv.now.count<<"_______"<<endl;
}
void set_last(content *cont,tsend &Bsend){  //将发送需要的数据写入发送消息中
    if(Bsend.now.row!=-1&&Bsend.now.count!=-1){
        Bsend.value = cont->value[Bsend.now.row];
        Bsend.weight = cont->weight[Bsend.now.row];
        memcpy(Bsend.last_date,cont->dp[(Bsend.now.row+1)%2],sizeof(long long)*(package_size + 1));
    }
}
void R_step(content *cont){//主线程分发任务
    ...
    int Bsend_size;
    int Size;
    MPI_Pack_size(1,tsend_type,MPI_COMM_WORLD,&Bsend_size);
    Size = (numprocs + 1) * (MPI_BSEND_OVERHEAD + Bsend_size);
    buf = malloc(Size);
    ...
    while(Status.next_livelast.row<item_N){//只要任务没有分发完成，就循环
        ...
        //将该处理器放入未分配任务的处理器中
        add_node(Status,Bsend.now);
        //获取任务
        while(Bsend.now.count!=-1&&!Que.empty()){//在处理器分配完或不能分发新任务前一直循环
            set_last(cont,Bsend);
            //封装发送消息
            time.Start();
            MPI_Bsend(&Bsend,1,tsend_type,Que.front(),0,MPI_COMM_WORLD);
            time.End();
            c++;
            Sum+=time.costTime;
            // cout<<"Bsend cost time:"<<time.costTime<<endl;
            //发送消息
            Que.pop();
            //将该处理器从未分配中取出
            if(!Que.empty())//如果存在未分配的处理器时，在创建任务
                add_node(Status,Bsend.now);
        }
    }
    // Send cost time:32.7596
    // Bsend cost time:26.0969
    cout<<"Bsend cost time:"<<(double)Sum/c<<endl;
    for(int i=1 ; i < numprocs ; i++){//将所有处理器进程退出
        MPI_Recv(&Bresv,1,tresv_type,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&mpi_S);
        Bsend.now.row = Status.next_livelast.row;
        Bsend.now.count = Status.next_livelast.count;
        MPI_Send(&Bsend,1,tsend_type,mpi_S.MPI_SOURCE,0,MPI_COMM_WORLD);
    }
}
int main(int argc,char* argv[]){
    ...
    if(myid==0){        //通过0进程读取文件，并且搭建内存共享
        ...
        // for(int i: cont->value)cout<<i<<" ";cout<<endl;
    }
    if(myid==0){
        cout<<"开始监听"<<endl;
        MyTimer time;
        ...
        R_step(cont);
        time.End();
        cout<<"ans :"<<cont->dp[(item_N - 1)%2][package_size]<<endl;
